...
RAB_DATASET_BASE_PATH = '/home/tuyunbin/Video-Description-with-Spatial-Tempral-Attention/msvd_data/'
# the dir where all the experiment data is dumped
#
# .the dir where store your experimental results
RAB_EXP_PATH = '/home/sdc/tuyunbin/msvd_result/Video-Description-with-Spatial-Tempral-Attention/exp/'
...
def adam(lr, tparams, grads, inp, cost,extra):
    gshared = [theano.shared(p.get_value() * numpy.float32(0.), name='%s_grad'%k) for k, p in tparams.iteritems()]
    gsup = [(gs, g) for gs, g in zip(gshared, grads)]

    f_grad_shared = theano.function(inp, [cost]+extra, updates=gsup,on_unused_input='ignore', profile=False)
