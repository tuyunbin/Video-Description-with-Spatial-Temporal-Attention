...
def generate_sample_gpu_single_process(
    ...
    def sample(whichset):
        samples = []
        ctxgs,ctxg_masks,ctxls, ctxl_masks,ctxms,ctxm_masks = engine.prepare_data_for_blue(whichset)
        for i, ctxg,ctxg_mask,ctxl, ctxl_mask,ctxm,ctxm_mask in zip(range(len(ctxgs)), ctxgs,ctxg_masks,ctxls, ctxl_masks,ctxms,ctxm_masks):
            print 'sampling %d/%d'%(i,len(ctxgs))
            sample, score, _, _ = model.gen_sample(
                None, f_init, f_next, ctxg,ctxg_mask,ctxl, ctxl_mask, ctxm,ctxm_mask,options,
